...
# ---------------- Page config ----------------
st.set_page_config(page_title="Şans Topu Tahmin Botu — Clean",
                   page_icon="🎯", layout="centered")

# ---------------- Helpers ----------------
@st.cache_data(ttl=3600)
def get_time_weights(dates, halflife_days=180):
...
def compute_single_freq(df, halflife_days=180):
    w = get_time_weights(df['Date'], halflife_days)
...
def model_pattern_score_shans(combo):
    ...
    return 1.2 if max(ranges) <= 2 else 1.0
...
def ensure_joker_freq(joker_freq):
    try:
        if joker_freq is None or len(joker_freq) != 14 or np.isnan(joker_freq.sum()):
            return pd.Series(1/14, index=range(1,15))
        jk = joker_freq.reindex(range(1,15), fill_value=0).astype(float)
        if jk.sum() <= 0:
            return pd.Series(1/14, index=range(1,15))
        return jk / jk.sum()
    except Exception:
...
def generate_predictions_with_models(df, single_prob, pair_freq, cond_prob, transitions,
                                     ...
                                     n_sets=4, trials=4000, alpha=0.35, beta=0.25, gamma=0.25, delta=0.15):
    results = []
    ...
    joker_freq = ensure_joker_freq(joker_freq)
    ...
    for s in range(n_sets):
        ...
        try:
            ...
            jk_probs = np.array(jk_probs, dtype=float)
            if jk_probs.sum() <= 0 or np.any(np.isnan(jk_probs)):
                jk_probs = np.ones(14) / 14.0

            nb_dist = np.ones(14) / 14.0
            if joker_nb is not None and hasattr(joker_nb, 'predict_proba'):
                try:
                    ...
                    nb_dist = np.zeros(14, dtype=float)
                    for idx, c in enumerate(classes):
                        ci = int(c)
                        if 1 <= ci <= 14:
                            nb_dist[ci-1] = jk_nb_probs[idx]
                    if nb_dist.sum() <= 0 or np.any(np.isnan(nb_dist)):
                        nb_dist = np.ones(14) / 14.0
                    else:
            ...
            if np.any(np.isnan(combined)) or combined.sum() == 0:
                combined = np.ones(14) / 14.0
            combined = combined / combined.sum()

            joker_pick = int(np.random.choice(range(1,15), p=combined))
        except Exception:
            try:
                if 'Joker' in df.columns and not df['Joker'].isna().all():
                    joker_pick = int(df['Joker'].mode().iloc[0])
                else:
...
# ---------------- Streamlit App ----------------
def main():
    st.title("Şans Topu Tahmin Botu — Clean (Joker destekli)")
    ...
    trials = st.sidebar.number_input("Her set için deneme sayısı", min_value=500, max_value=30000, value=4000, step=500)
    ...
    if 'joker_freq' not in st.session_state:
        st.session_state['joker_freq'] = None
    ...
    joker_col = next((c for c in raw.columns if 'joker' in c.lower()), None)
